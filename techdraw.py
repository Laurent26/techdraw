...
class OBJECT_OT_ApplyButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        resultRef = resultRef and (bpy.context.scene.Ref.name in context.view_layer.objects)
...
class OBJECT_OT_UpdateButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        resultUpdate = resultUpdate and (bpy.context.scene.Update.name in context.view_layer.objects)
...
class OBJECT_OT_FormatButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if formatCartridge != "None":
            ...
            bpy.data.collections[sheets].children.link(bpy.data.collections[formatCartridge])
            master_collection = bpy.context.scene.collection 
            master_collection.children.unlink(bpy.data.collections[formatCartridge])
...
class OBJECT_OT_GridAddButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        resultGrid = resultGrid and (bpy.context.scene.Target_Table.name in context.view_layer.objects)
        ...
        if (resultGrid == True) and (resultCollecGrid == True):            
            ...
            bpy.ops.mesh.primitive_grid_add(x_subdivisions=xSubdivisions, y_subdivisions=ySubdivisions, size=xDimTarget, enter_editmode=True, location=(0, 0, 0))                        
            bpy.ops.mesh.select_all(action='SELECT')
            bpy.ops.mesh.mark_freestyle_edge(clear=False)
            bpy.ops.object.editmode_toggle()
            
            bpy.context.object.dimensions[1] = 0.007 * (ySubdivisions - 1) * scaleY
            bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
            
            if context.window_manager.orignGridState == 1:
                bpy.context.scene.cursor.location[0] = xDimTarget / 2
                bpy.context.scene.cursor.location[1] = ((-0.007) * (ySubdivisions-1) * scaleY) / 2
            ...
            principled_bsdf = materialGrid.node_tree.nodes.get('Principled BSDF')
            ...
            if textGridBool == True:
                ...
                principled_bsdf = materialFont.node_tree.nodes.get('Principled BSDF')
                ...
                for l in range(ySubdivisions - 1):
                    for c in range(xSubdivisions - 1):
                        bpy.ops.object.text_add(radius=dimTxt, enter_editmode=False, location=(0, 0, 0))
                        bpy.context.object.location[0] = (c * ((xDimTarget)/(xSubdivisions - 1))) + (0.001 * scaleY) - locXText
                        bpy.context.object.location[1] = (l * (0.007 * scaleY)) + (0.002 * scaleY) - locYText
...
class OBJECT_OT_CutAddButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        resultCut = resultCut and (bpy.context.scene.Cutter.name in context.view_layer.objects)
        resultParts = not bpy.context.scene.Parts is None
        resultParts = resultParts and (bpy.context.scene.Parts.name in context.view_layer.objects)
        ...
        if (resultCut == True) and (resultParts == True):
            ...
            if bpy.data.objects[target_parts.name].active_material is None:
                ...
                principled_bsdf = materialParts.node_tree.nodes.get('Principled BSDF')            
    ...
    def execute(self, context):
        ...
        if resultCut == True:
            ...
            if drawingType != 4:
                ...
                principled_bsdf = materialCut.node_tree.nodes.get('Principled BSDF')            
                ...
                if drawingType == 2:
                    ...
                    colorRamp = materialCut.node_tree.nodes.new('ShaderNodeValToRGB')
                    if colorRamp is not None:
                        colorRamp.color_ramp.elements[0].position = 0.06
                        colorRamp.color_ramp.elements[1].position = 0.2
                            
                    mapping = materialCut.node_tree.nodes.new('ShaderNodeMapping')
                                    
                    textureCoord = materialCut.node_tree.nodes.new('ShaderNodeTexCoord')
                    
                    if hatchType == "Magic":
                        textureHatch = materialCut.node_tree.nodes.new('ShaderNodeTexMagic')
                    ...
                    if hatchType == "Musgrave":
                        textureHatch = materialCut.node_tree.nodes.new('ShaderNodeTexMusgrave')
                        if textureHatch is not None:
                    ...
                    if hatchType == "Noise":
                        textureHatch = materialCut.node_tree.nodes.new('ShaderNodeTexNoise')
                        if textureHatch is not None:
                            textureHatch.inputs[1].default_value = 20.0
                    ...
                    if hatchType == "Voronoi":
                        textureHatch = materialCut.node_tree.nodes.new('ShaderNodeTexVoronoi')
                        if textureHatch is not None:
                    ...
                    if hatchType == "Brick":
                        textureHatch = materialCut.node_tree.nodes.new('ShaderNodeTexBrick')
                        if textureHatch is not None:
                            textureHatch.inputs[4].default_value = 10.0
                            textureHatch.inputs[5].default_value = 0.015
                    
                    if hatchType == "Wave":
                        textureHatch = materialCut.node_tree.nodes.new('ShaderNodeTexWave')
                        if textureHatch is not None:
                            textureHatch.inputs[1].default_value = 20.0
                    ...
                    materialCut.node_tree.links.new(colorRamp.outputs[0], principled_bsdf.inputs[0])
                    materialCut.node_tree.links.new(textureHatch.outputs[0], colorRamp.inputs[0])
                    materialCut.node_tree.links.new(mapping.outputs[0], textureHatch.inputs[0])
                    materialCut.node_tree.links.new(textureCoord.outputs[2], mapping.inputs[0])
                ...
                if (img_formats == True):        
                    if drawingType == 3:
                        ...
                        imageDraw = materialCut.node_tree.nodes.new('ShaderNodeTexImage')
                        if imageDraw is not None:
                            imageDraw.image = bpy.data.images.load(texturePath)
                        
                        materialCut.node_tree.links.new(imageDraw.outputs[0], principled_bsdf.inputs[0])
                        materialCut.node_tree.links.new(mapping.outputs[0], imageDraw.inputs[0])
                        materialCut.node_tree.links.new(textureCoord.outputs[2], mapping.inputs[0])
...
class OBJECT_OT_DupliBoolButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.object.duplicate_move(OBJECT_OT_duplicate={"linked":False, "mode":'TRANSLATION'}, TRANSFORM_OT_translate={"value":(xValueC, yValueC, zValueC)})
...
class OBJECT_OT_SnapPartsButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        resultS1 = resultS1 and (bpy.context.scene.S1.name in context.view_layer.objects)
        resultS2 = not bpy.context.scene.S2 is None
        resultS2 = resultS2 and (bpy.context.scene.S2.name in context.view_layer.objects)
        resultS3 = not bpy.context.scene.S3 is None
        resultS3 = resultS3 and (bpy.context.scene.S3.name in context.view_layer.objects)
        resultS4 = not bpy.context.scene.S4 is None
        resultS4 = resultS4 and (bpy.context.scene.S4.name in context.view_layer.objects)
        ...
        if (resultS1 == True) and (resultS2 == True) and (resultS3 == True) and (resultS4 == True):
            ...
            for o in listObj:
                ...
                if i == 1:
                    locObj1 = [xmin + ((xmax - xmin)/2), ymin + ((ymax - ymin)/2), zmin + ((zmax - zmin)/2)]
                    bpy.context.scene.cursor.location[0] = xmin + ((xmax - xmin)/2)
                    bpy.context.scene.cursor.location[1] = ymin + ((ymax - ymin)/2)
                    bpy.context.scene.cursor.location[2] = zmin + ((zmax - zmin)/2)                        
                    bpy.ops.object.origin_set(type='ORIGIN_CURSOR', center='MEDIAN')
                if i == 2:
                    locObj2 = [xmin + ((xmax - xmin)/2), ymin + ((ymax - ymin)/2), zmin + ((zmax - zmin)/2)]
                    bpy.context.scene.cursor.location[0] = xmin + ((xmax - xmin)/2)
                    bpy.context.scene.cursor.location[1] = ymin + ((ymax - ymin)/2)
                    bpy.context.scene.cursor.location[2] = zmin + ((zmax - zmin)/2)                        
                    bpy.ops.object.origin_set(type='ORIGIN_CURSOR', center='MEDIAN')
                if i == 3:
                    locObj3 = [xmin + ((xmax - xmin)/2), ymin + ((ymax - ymin)/2), zmin + ((zmax - zmin)/2)]
                    bpy.context.scene.cursor.location[0] = xmin + ((xmax - xmin)/2)
                    bpy.context.scene.cursor.location[1] = ymin + ((ymax - ymin)/2)
                    bpy.context.scene.cursor.location[2] = zmin + ((zmax - zmin)/2)                        
                    bpy.ops.object.origin_set(type='ORIGIN_CURSOR', center='MEDIAN')
                if i == 4:
                    locObj4 = [xmin + ((xmax - xmin)/2), ymin + ((ymax - ymin)/2), zmin + ((zmax - zmin)/2)]
                    bpy.context.scene.cursor.location[0] = xmin + ((xmax - xmin)/2)
                    bpy.context.scene.cursor.location[1] = ymin + ((ymax - ymin)/2)
                    bpy.context.scene.cursor.location[2] = zmin + ((zmax - zmin)/2)                        
                    bpy.ops.object.origin_set(type='ORIGIN_CURSOR', center='MEDIAN')
...
class OB_PT_LSToolsPanelLayout(bpy.types.Panel):
    ...
    bpy.types.Scene.Collection_Table = PointerProperty(type=bpy.types.Collection)
